...
def download_image(id, img_url, output_dir):
    try:
        # Open the URL and read the content
        with request.urlopen(img_url, timeout=10) as response:
            if response.getcode() == 200:

                parsed_url = urlparse(img_url)
                filename = os.path.join(output_dir, f"{str(id)}{os.path.splitext(parsed_url.path)[1]}")
...
with open(csv_file, 'r', newline='', encoding='utf-8') as file:
    ...
    for row in reader:
        ...
        #print(img_url)
        # Download the image
        if download_image(index, img_url, output_dir):
            index+=1
